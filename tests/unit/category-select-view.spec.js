...
it('displays CategorySelectView component', () => {
  ...
  it('toggles "All Selected" option', async () => {
    const wrapper = mount(CategorySelectView);
    const allSelectedOption = wrapper.find('.container option[value="All Selected"]');
    
    await allSelectedOption.trigger('click'); // Click on "All Selected" option
    expect(wrapper.vm.selections).toEqual(['Italian Food', 'French Food', 'Asian Food', 'Eastern Food']);
